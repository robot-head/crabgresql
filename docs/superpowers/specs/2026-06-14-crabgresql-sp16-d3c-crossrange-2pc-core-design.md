...
## Implementation refinements (vs. this spec) — recorded

The implementation resolved two open questions and deferred some spec-listed pieces to SP17. None weaken the slice's guarantee (atomic commit + SI, proven by the cross-range `jepsen_bank` conservation oracle under a crash/partition nemesis):

- **Escalation mechanism (resolved in the plan, not buffering):** tuples keep their per-range **local xid `Li`**; a `Prepared(Li → G)` clog marker ties each to the global txn, and **`Li` is deregistered from its range's ProcArray running-set at prepare time** so the local snapshot stops gating prepared rows and range 0's global clog is the sole arbiter (both ranges flip atomically at the single `Committed(G)`). No re-stamping, no write-buffering; single-range is byte-for-byte unchanged. (This replaced the spec's "buffer + re-stamp" sketch.)
- **EvalPlanQual under the row lock (bug found by the bank oracle, fixed):** an UPDATE/DELETE re-check resolves a `Prepared(Li → G)` deleter against the **current global clog directly** (a read-committed-under-lock view), not the statement's pre-lock global snapshot — because the row lock guarantees any global txn that touched the row has already committed/aborted. Without this, a concurrent cross-range commit landing while a writer blocked on the lock caused a lost update.
- **Deferred to SP17 (the cross-node slice), not dropped:** the durable transaction record (`/0/txn/<G>`) + the active range-0 **recovery sweep** (Component 1 / the spec's recovery decision) — in-process the only "crash" is a coordinator `RangeRouter` drop, which self-releases participant locks via `SqlSession::Drop`, so presumed-abort + the global-clog-as-decision satisfies criterion 5 for the in-process fault model; the durable record/sweep are needed only when a real node crash strands a *remote* participant's locks. Also deferred: the `ServerNode`/`serve_routed`/`gateway_local` `0A000`→atomic-commit flip (it needs the cross-node global-decision path — a cross-range txn whose gateway does not lead range 0); SP16 flips the in-process `RangeRouter` test (`a_cross_range_transaction_commits_atomically`) instead, and gates escalation on the coordinator carrying the GTM so the not-yet-wired `serve_routed` path keeps its `0A000` rejection. And **jepsen_elle cross-range** (spec T6, optional) → SP17.

## Traceability (implemented)

| # | Criterion | Verified by |
|---|---|---|
| 1 | `Prepared(G)` round-trips; resolver routes local vs. global; `Prepared` invisible | `mvcc::clog::tests::prepared_carries_global_xid_roundtrip` + `executor::exec::tests::global_status_derefs_prepared_to_range0_global_clog` (T1/T3) |
| 2 | Range 0 allocates monotonic disjoint global xids; survives leadership change | `executor::gtm::tests::{allocates_disjoint_monotonic_global_ids, reseed_lifts_counter_and_never_regresses}` (T2) |
| 3 | Cross-range `BEGIN…COMMIT` to two tables in two ranges commits atomically; abort leaves neither | `cluster::range::router::tests::{a_cross_range_transaction_commits_atomically, a_cross_range_transaction_rolls_back_atomically}` (T4) |
| 4 | Prepared-but-undecided rows invisible to a concurrent reader until `Committed(G)` | `cluster::range::router::tests::coordinator_pause_seam_holds_a_txn_in_doubt` (T4) |
| 5 | Coordinator crash between staging and the decision resolves atomically (presumed-abort / commit) | `cluster::range::router::tests::{crash_before_global_decision_presumes_abort, crash_after_global_decision_commits}` (T5) |
| 6 | `jepsen_bank` with accounts in two tables in different ranges conserves the total under a crash/partition nemesis | `cluster::jepsen_bank::{cross_range_bank_conserves_total_serial, cross_range_bank_conserves_total_under_nemesis}` (T5) |
| 7 | Single-range MVCC/txn suites pass unchanged; single-range never touches the global machinery | full `cargo nextest run --workspace` green (T1–T5 regression gate; 119 executor+mvcc, 83 cluster) |
| 8 | Former cross-range `0A000` rejection flipped to atomic commit (in-process); cross-range UPDATE exercises EvalPlanQual; no new shipped dep; full gauntlet | `cluster::crossrange_2pc::cross_range_update_commits_atomically` + T4 flip + T7 gauntlet (the `gateway_local`/`ServerNode` flip → SP17) |
